    ...
    for hfile in hfiles:
        ...
        with open(hfile) as f:
            for line in f:
                ...
                #User extraction
                User.append(line.split(" ")[3-1])
                #PR extraction
                PR.append(line.split(" ")[4-1])
                #Status extraction
                S.append(line.split(" ")[7-1])
                #The process run on which CPU core
                CPU.append(int(line.split(" ")[15-1]))
                #SysTime extraction
                SysT.append(float(line.split(" ")[16-1]))
                #UserTime extraction
                UsrT.append(float(line.split(" ")[17-1]))
                #Percentage of the current interval taken up by this task
                Pct.append(int(line.split(" ")[18-1]))
                #Command extraction
                Command.append(node + "-" +line.split(" ")[29-1].strip())
